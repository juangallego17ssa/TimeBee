...
function DataProject() {
  ...
  useEffect(() => {
    ...
    const getTrackedTime = async () => {
      const response = await axiosWithToken(
        `trackedtime/?start_date=${startDateString}&end_date=${endDateString}`
      );
  ...
  for (const day in dataTimeOwn) {
    dataTimeOwn[day] = dataTimeOwn[day].sort(
    ...
    const projectDurations = dataTimeOwn[day].reduce((acc, item) => {
      ...
      const hours = Math.abs(
        parseFloat((duration.asMinutes() / 60).toFixed(2))
      ); // hours in decimal round to 2 digits
  ...
  return (
    // Create Barchart
    <div className=" Container flex flex-col flex-grow bg-stone-100 px-8 py-4 h-full w-full rounded-3xl">
      <div>
        <select
