...
const EfficientTime = () => {
  ...
  function formatTimeFromMinutes2(totalMinutes) {
    ...
    return (
      ...
      " " +
      (minutes ? minutes.toString().padStart(2, "0") + " min" : "00 min")
    );
  }
  ...
  const getMonthInfo = async (myDay) => {
    ...
    try {
      ...
      setWeekNum(
        `${getMonthName(monthNum).toString()} - ${weekDate.getFullYear()}`
      );
      ...
      for (let month of response.data.detail_monthly) {
        if (month.month === monthNum + 1) {
          ...
          setWeekBreakTime(
            formatTimeFromMinutes(
              Math.round(
                (month.time_span_average - month.duration_average) / 60
              )
            )
          );
          setWeekWorkTime(
            formatTimeFromMinutes(Math.round(month.duration_average / 60))
          );
          setWeekBreakNum(month.breaks_average.toFixed(1));
          setWeekBreakTimePerBreak(
            formatTimeFromMinutes2(
              Math.round(
                (month.time_span_average - month.duration_average) /
                  60 /
                  month.breaks_average
              )
            )
          );
          const dataArray = [];
  ...
  return (
    <div className="h-full w-full p-5">
      ...
      <div className="flex justify-end mt-1">
        <p className="tracking-wider text-sm">Show result for:</p>
        <select
          className="text-sm ml-4 bg-teal-400 text-white rounded-2xl"
          id="selector"
          id="weekMonthSelector"
          name="selector"
      ...
      {/* Average  */}
      <div className="flex-col mt-3 shadow-sm shadow-gray-400 p-1">
        <div className="flex w-full px-8">
          <div
            className="flex justify-center items-center cursor-pointer w-1/12 semi-bold text-sm text-white rounded bg-zinc-500"
            onClick={handleClickLeft}
          >
            &lt;&lt;
          </div>
          <div className="flex justify-center items-center w-10/12 semi-bold text-[18px] text-zinc-600">
            {weekNum}
          </div>
          <div
            className="flex justify-center items-center cursor-pointer w-1/12 semi-bold text-sm text-white rounded bg-zinc-500"
        ...
        <div className="EfficientTimeChart1 pt-6 px-8 h-40 flex items-center justify-center w-full">
          ...
          {/*</div>*/}
          <div className="flex flex-col h- pl-2">
            <div className="flex flex-col justify-center items-center h-1/5 text-sm">
              <p className="tracking-wider text-xs text-zinc-400">
                Av. break time:
              </p>
            ...
            <div className="flex flex-col justify-center items-center h-4/5 text-sm ">
              <p className="tracking-wider text-xs text-teal-400">
                Av. work time:
              </p>
      ...
      {/* Big numbers */}
      <div className="breakInfo mt-3 flex items-center justify-center">
        <div className="h-18 w-18 p-3  border border-solid text-zinc-500 border-zinc-300 flex justify-center items-center">
      ...
      <div className="breakInfo mt-2 flex items-center justify-center">
        <div className="h-18 w-18 p-3 l border border-solid text-zinc-500 border-zinc-300 flex justify-center items-center">
          <p className="text-3xl flex items-center justify-center text-center align-center">
            {weekBreakTimePerBreak}
          </p>
        </div>
        <div className="flex text-zinc-600 items-center justify-center ml-5 text-sm">
          <p>AV. TIME/BREAK</p>
      ...
      <div className="mt-3 shadow-sm shadow-gray-400" >
