...
class ListTrackedTimeByCodeView(ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if start_date_str and end_date_str:
            ...
            if type_of_input:
                queryset = queryset.filter(Q(type_of_input=self.request.query_params.get('type_of_input')) & Q(start__date__gte=start_date, start__date__lte=end_date))
            else:
                queryset = queryset.filter(Q(code=self.request.query_params.get('code')) & Q(start__date__gte=start_date, start__date__lte=end_date))
